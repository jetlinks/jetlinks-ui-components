...
;(function (window) {
  function MvtImageryProvider(options) {
    ...
    switch (options.style) {
      case 'mapbox-streets-v6':
        this._styleClass = new sxii.MapboxStreetsV6()
        break
      default:
  ...
  class MvtLayer extends sxii.layer.BaseTileLayer {
    //构建ImageryProvider
  ...
  sxii.LayerUtil.register(layerType, MvtLayer)
  sxii.LayerUtil.registerImageryProvider(layerType, createImageryProvider)

  //对外接口
  window.sxii.provider.MvtImageryProvider = MvtImageryProvider
  window.sxii.layer.MvtLayer = MvtLayer
})(window)
