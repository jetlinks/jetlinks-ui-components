 ...
 */
export const termsParamsFormat = (terms, columnOptionMap, type:string = 'adv') => {
    ...
    if (type === 'adv') {
        ...
        return cloneParams.filter(iItem => iItem && iItem.value).map(iItem => handleItemValue(iItem, columnOptionMap))
    }
}
